...
object ItemsPersistenceAnorm extends AnormPersistence with ItemsPersistence {
  ...
  val winningBidByIdSQL = SQL(winningBidSQL + """ where "id" = {id} """.stripMargin)
  ...
  val winningBidsByBidderSQL = SQL(winningBidSQL + """ where "bidder_id" = {bidder_id} """.stripMargin)
  ...
  val winningBidsByItemSQL = SQL( """ where "item_id" = {item_id} """.stripMargin)
